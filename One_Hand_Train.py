...
def Segmentation(Data_EMG, Data_ACC, Data_GYR, Num_Seg):
    ...
    for Index_SLR in range(len(Data_EMG)):
        ...
        for i in range(Num_Seg):
            ...
            Seg_ACC.append(Length_Adjust(Single_ACC[Index:Index + Num[i + 1], :], 160))
            Seg_GYR.append(Length_Adjust(Single_GYR[Index:Index + Num[i + 1], :], 160))
            Index = Index + Num[i + 1]
...
def Folder_combination(Path, Num_Folder):
    for Index_Folder in range(Num_Folder):
        EMG_Data, ACC_Data, GYR_Data = Load_ALL_Data(Path, SIGN_COUNT, 9, 3, 3, Index_Folder + 1)
...
def Pre_processing():
    Path = os.getcwd()
    Seg_EMG, Seg_ACC, Seg_GYR = Folder_combination(Path, BATCH_AMOUNT)
